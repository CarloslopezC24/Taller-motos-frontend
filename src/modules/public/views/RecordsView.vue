<script setup>
    ...
    async function dispararNotificacion(nombre) {
        try {
            await fetch('http://127.0.0.1:5000/enviar-alerta', {
                ...
                body: JSON.stringify({
                    asunto: "Nueva Orden de Servicio - Taller",
                    mensaje: `Se ha registrado exitosamente un nuevo servicio/producto en el sistema: ${nombre}`,
                    destinatario: "cabreracarlos1415@gmail.com"
                })
            });
        } catch (e) { 
            console.error("Error al enviar el correo de notificación:", e); 
        }
    }
    ...
    async function agregarRegistro() {
      ...
      const confirmar = window.confirm("¿Seguro que deseas agregar este registro y enviar la notificación?");
      if (!confirmar) return;
      ...
      // Respaldamos el nombre antes de limpiar el formulario para evitar que el correo viaje vacío
      const nombreServicio = form.nombre;
      ...
      try {
        ...
        // 1. Envío de correo en la nube automático tras éxito en base de datos
        await dispararNotificacion(nombreServicio);

        // 2. Forzar recarga asíncrona para pintar el registro nuevo en la interfaz
        await recordsStore.cargarRegistros();

        mensajeEnvio.value = "Registro del taller guardado correctamente en la base de datos"
        form.nombre = ''
        form.categoria = ''
        form.descripcion = ''
        form.precio = 1
  ...
  async function confirmarDesactivacion(item) {
    ...
    try {
      ...
      // Recargar registros tras eliminación lógica para refrescar los componentes
      await recordsStore.cargarRegistros();
    } finally {
...
<template>
    <section>
        <h2>Registros de Culiacán Bikers Garage</h2>
...
input, select, button {
  ...
  border-radius: 10px;
  border: 2px solid rgb(78, 32, 192);
  font-size: 14px;
}
...
button {
  border: none;
  background: linear-gradient(90deg, #21ac5b, #630404);
  color: white;
  font-weight: bold;
  cursor: pointer;
...
.sin-contenido {
  ...
  text-align: center;
  color: #666;
  box-shadow: 0 5px 12px rgba(189, 70, 70, 0.1);
} 
...
.textarea-descripcion {
  ...
  padding: 12px;
  border-radius: 10px;
  border: 2px solid rgb(35, 10, 197);
  font-size: 14px;
  box-sizing: border-box;
}
